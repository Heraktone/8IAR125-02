...
Vehicle::Vehicle(GameWorld* world,
	...
	m_dTimeElapsed(0.0), renderingColor(RenderColor::None)
{
	InitializeBuffer();
...
Vehicle::Vehicle(GameWorld * world, Vector2D position, double rotation, Vector2D velocity, double mass, double max_force, double max_speed, double max_turn_rate, double scale, RenderColor color) : Vehicle(world, position, rotation, velocity, mass, max_force, max_speed, max_turn_rate, scale)
{
...
void Vehicle::Render()
{
	...
	switch (renderingColor)
	{
	case Vehicle::RenderColor::Player:
		gdi->GreenPen();
		customColor = true;
		break;
	case Vehicle::RenderColor::Leader:
		gdi->RedPen();
		customColor = true;
		break;
	case Vehicle::RenderColor::Follower:
		gdi->BluePen();
		customColor = true;
		break;
	case Vehicle::RenderColor::None:
		break;
	default:
		break;
	}
